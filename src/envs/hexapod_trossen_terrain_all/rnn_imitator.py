...
def imitate_multiple(n_classes, iters):
    env = hex_env.Hexapod()
    master = policies.RNN_V3_LN_PG(env, hid_dim=64, memory_dim=32, n_temp=3, tanh=True, to_gpu=True).cuda()
    ...
    expert_states = np.load("data/states_REACTIVE.npy")
    # N x EP_LEN x ACT_DIM
    expert_acts = np.load("data/acts_REACTIVE.npy")
