...
if __name__=="__main__":
    ...
    heightfield_height = [0.02, 0.04, 0.08, 0.2]

    simulation_time = 30 #[s] the length of a single experiment

    
    first_run = True
    for height in heightfield_height:
        for experiment in experiments:
            ...
            #run the vrep headless
            scenario = "irregular_" + str(height) + ".ttt"
            subprocess.Popen(['vrep', '-h -q terrain_scenes/' + scenario])
            time.sleep(4)
            ...
            logfile = "logs/experiment_" + used_gait + "_" + str(swing_length) + "_" + str(swing_height) + "_" + str(height) + ".log"
